...
import 'package:disys_network/disys_network.dart' as disys_network;
...
final _logger = Logger('Network Generator');

class NetworkGenerator extends GeneratorForAnnotation<disys_network.NetworkApi> {
  @override
  FutureOr<String> generateForAnnotatedElement(
    ...
    if (element is! ClassElement) {
      ...
      throw InvalidGenerationSourceError(
        ...
        todo: 'Remove the [NetworkApi] annotation from `$friendlyName`.',
      );
    }

    return _buildNetworkApiImplementationClass(annotation, element);
  }

  bool _extendsNetworkService(InterfaceType type) =>
      _typeChecker(disys_network.NetworkService).isExactlyType(type);

  code_builder.Field _buildDefinitionTypeMethod(String superType) => code_builder.Field(
        (method) => method
  ...
  String _buildNetworkApiImplementationClass(
    ConstantReader annotation,
    ClassElement element,
  ) {
    if (!element.allSupertypes.any(_extendsNetworkService)) {
      final friendlyName = element.displayName;
      throw InvalidGenerationSourceError(
        'Generator cannot target `$friendlyName`.',
        todo: '`$friendlyName` need to extends the [NetworkService] class.',
      );
    }
  ...
  Constructor _generateConstructor() => Constructor((constructorBuilder) {
        constructorBuilder.optionalParameters.add(
          Parameter((paramBuilder) {
            ...
            paramBuilder.type = refer('${disys_network.NetworkClient}');
          }),
        );
  ...
  Iterable<code_builder.Method> _parseMethods(ClassElement element, String baseUrl) {
  ...
  code_builder.Method _generateMethod(MethodElement m, String baseUrl) {
    final method = _getMethodAnnotation(m);
    final multipart = _hasAnnotation(m, disys_network.Multipart);
    final factoryConverter = _getFactoryConverterAnnotation(m);

    final body = _getAnnotation(m, disys_network.Body);
    final paths = _getAnnotations(m, disys_network.Path);
    final queries = _getAnnotations(m, disys_network.Query);
    final queryMap = _getAnnotation(m, disys_network.QueryMap);
    final fields = _getAnnotations(m, disys_network.Field);
    final fieldMap = _getAnnotation(m, disys_network.FieldMap);
    final parts = _getAnnotations(m, disys_network.Part);
    final partMap = _getAnnotation(m, disys_network.PartMap);
    final fileFields = _getAnnotations(m, disys_network.PartFile);
    final fileFieldMap = _getAnnotation(m, disys_network.PartFileMap);
  ...
  ConstantReader? _getFactoryConverterAnnotation(MethodElement method) {
    final annotation = _typeChecker(disys_network.FactoryConverter)
        .firstAnnotationOf(method, throwOnUnresolved: false);
    if (annotation != null) return ConstantReader(annotation);
    return null;
  }
  ...
  final _methodsAnnotations = const [
    disys_network.Get,
    disys_network.Post,
    disys_network.Delete,
    disys_network.Put,
    disys_network.Patch,
    disys_network.Method,
    disys_network.Head,
    disys_network.Options,
  ];
  ...
  Code? _generateHeaders(MethodElement methodElement, ConstantReader method) {
    ...
    final annotations = _getAnnotations(methodElement, disys_network.Header);
...
Builder networkGeneratorFactoryBuilder({String? header}) => PartBuilder(
      [NetworkGenerator()],
      '.disys_network.dart',
      header: header,
    );
